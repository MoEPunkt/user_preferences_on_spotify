...
data <- charts %>%
  ...
  group_by(id, region) %>%
  # compute total streams per track and regions
  ...
  left_join(dates, by = c("region", "id")) %>%
...
data <- select(data,
               c(-id, -artist_id, -main_artist, -artist_total_streams,
                 -artist_streams_per_capita, -min_date, -max_date, -n_days))
data <- data[-1]
...
save(m_xgb, file = "output/m_xgb.RData")

# predict outcome on test data
p <- predict(m_xgb, test[,-1])
